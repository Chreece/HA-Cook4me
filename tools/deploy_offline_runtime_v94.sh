#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v94.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=7d9250c2a0dee825e53949d2a073c2262d51d100
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v94-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 102),
        ("price_portions.v1.json", "portions", 90),
        ("price_reference_portions.v1.json", "portions", 4),
        ("price_densities.v1.json", "densities", 2),
    ...
    assert count == 473, f"Merged offline price snapshot: expected 473 observations, found {count}"
    measurements = importlib.import_module(f"{package.__name__}.price_measurements")
    for name, quantity, expected in (("Rapeseed oil", 40, 40 / .92), ("Balsamic vinegar", 20, 18.75)):
        options = measurements.price_options({"name": name, "quantity": quantity, "unit": "g"})
        assert any(row["unit"] == "ml" and abs(row["quantity"] - expected) < 0.001 for row in options), f"{name}: sourced volume conversion failed"
    assert any(row["unit"] == "g" and row["quantity"] == 400 for row in measurements.price_options({"name": "Small pumpkin", "quantity": 1})), "Small pumpkin reference missing"
    assert prices.snapshot_observations(category="en:pumpkins", country="DE", currency="EUR", unit="g"), "Pumpkin price missing"
...
printf 'Fetching tested Cook4me v94 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v94 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v94-bundle.js ]] || die 'Source is missing v94 runtime files.'
...
printf 'Running the price-conversion and cache checks for this build...\n'
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_conversions_v94.py -v
...
printf 'Waiting for Home Assistant and the v94 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.18/cook4me-panel-v94-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v94-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v94 panel within the startup timeout.'
