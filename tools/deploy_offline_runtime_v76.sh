#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v76.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=d17bc0c8627831c7ae5c4e54f098b3e8d13242dd
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v76-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v76 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v76-bundle.js ]] || die 'Source is missing v76 runtime files.'
...
printf 'Waiting for Home Assistant and the v76 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.1/cook4me-panel-v76-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v76-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v76 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app) after installing the v76 diet checks and complete ingredient replacements. Refresh Today, Week and Official recipes for the selected diet.\n'
