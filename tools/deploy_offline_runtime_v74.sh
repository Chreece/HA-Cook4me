#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v74.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=46a47c18a37487571be46107610bdd60010c4c55
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v74-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Reopen Cook4Me (reload the browser/app) for the v74 reconnect, recipe queue, saved-data and dashboard navigation fixes.\n'
