#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v86.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=849280cf7e6ee2e2af089f47532301a900f08900
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v86-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Waiting for Home Assistant and the v86 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.10/cook4me-panel-v86-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v86-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
