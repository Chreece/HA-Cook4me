...
Maintenance only: crawl every known official Cookeo language/market catalog,
hydrate each publication once, merge stable recipe/ingredient IDs, resolve
canonical English labels and generic per-100-g nutrition, then write the compact
repo index. Recipe steps and provider/account secrets are never persisted.
"""
...
import recipe_languages  # type: ignore  # noqa: E402
import cook4me_phonefree as c4m  # type: ignore  # noqa: E402
import cook4me_recipe_catalog as catalog  # type: ignore  # noqa: E402
from cook4me_official_nutrition import extract_official_nutrition  # type: ignore  # noqa: E402
...
def _all_search_rows(cfg, tokens, pcfg, *, language: str, country: str, configured_language: str, configured_country: str) -> list[dict[str, Any]]:
    ...
    while total_pages is None or page < total_pages:
        payload, _auth = catalog._http_json(
            ...
            params={
                "lang": language, "market": market, "page": page, "size": PAGE_SIZE,
                "q": "", "groupBy": "", "myUniverse": "false", "myOwnRecipe": "false",
...
def _detail(cfg, tokens, pcfg, *, variant_id: str, source_language: str, configured_language: str, configured_country: str) -> dict[str, Any]:
    variant = _fid(variant_id)
    url = cfg["platform_base_url"].rstrip("/") + "/common-api/v3/recipes/PRO/" + urllib.parse.quote(variant, safe="") + "/?format=mobile"
    payload, _auth = catalog._http_json(
        "GET", url,
        headers_iter=_headers(cfg, tokens, configured_country, configured_language, url, pcfg),
        timeout=30,
...
def _ingredient_id(item: dict[str, Any]) -> str:
    ...
    return key or "name:" + _text(item.get("foodName") or item.get("name")).casefold()


def _fdc_nutrient(food: dict[str, Any], tokens: tuple[str, ...], unit: str | None = None) -> float | None:
    for row in food.get("foodNutrients") if isinstance(food.get("foodNutrients"), list) else []:
...
def _fdc_profile(food: dict[str, Any], query: str) -> dict[str, Any] | None:
    values = {
        ...
        "carbohydrates": _fdc_nutrient(food, ("carbohydrate by difference", "carbohydrate")),
        "sugars": _fdc_nutrient(food, ("total sugars", "sugars total")),
        "fat": _fdc_nutrient(food, ("total lipid fat", "total fat")),
        "saturatedFat": _fdc_nutrient(food, ("fatty acids total saturated", "saturated fat")),
...
def _lookup_fdc(query: str, key: str) -> dict[str, Any] | None:
    body = json.dumps({"query": query, "pageSize": 8, "dataType": ["Foundation", "SR Legacy"]}).encode()
    request = urllib.request.Request(
        FDC_URL + "?api_key=" + urllib.parse.quote(key), data=body,
        headers={"Content-Type": "application/json", "User-Agent": "HA-Cook4me-release-builder/1"},
        method="POST",
    )
...
def _recipe_nutrition(ingredients: list[dict[str, Any]], nutrition: dict[str, dict[str, Any]], servings: Any) -> dict[str, Any]:
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for mapping in recipe_languages.SUPPORTED_RECIPE_LANGUAGES:
        language, country = mapping["code"], mapping["country"]
        print(f"[catalog] {language}/GS_{country}", flush=True)
        search_rows = _all_search_rows(
            cfg, tokens, pcfg, language=language, country=country,
            configured_language=args.configured_language, configured_country=args.configured_country,
        )
        unique = {
            _text(row.get("searchVariantId")): row
            for row in search_rows if _text(row.get("searchVariantId"))
        }
        hydrated = failed = 0
        with ThreadPoolExecutor(max_workers=max(1, min(args.workers, 8))) as pool:
            futures = {
                pool.submit(
                    _detail, cfg, tokens, pcfg, variant_id=variant_id,
                    source_language=language, configured_language=args.configured_language,
            ...
            for future in as_completed(futures):
                ...
                hydrated += 1
        stats.append({
            "language": language, "country": country, "rawPublications": len(search_rows),
            "uniqueVariants": len(unique), "hydratedVariants": hydrated, "failedDetails": failed,
        })

    variants = list(all_variants.values())
    ingredients: dict[str, dict[str, Any]] = {}
    for variant in variants:
        language = _text(variant.get("language") or variant.get("sourceCatalogLanguage")).lower()
        for item in variant.get("ingredients") or []:
            if not isinstance(item, dict):
                continue
            ident = _ingredient_id(item)
            name = _text(item.get("foodName") or item.get("name"))
            row = ingredients.setdefault(ident, {
                "key": _text(item.get("foodKey") or item.get("key")),
    ...
    overrides = _load_json(Path(args.english_overrides), {}) if args.english_overrides else {}
    ingredient_overrides = overrides.get("ingredients") if isinstance(overrides, dict) else {}
    for ident, row in ingredients.items():
        override = _text((ingredient_overrides or {}).get(ident)) if isinstance(ingredient_overrides, dict) else ""
        if override:
    ...
    if args.resolve_nutrition:
        for index, (ident, row) in enumerate(ingredients.items(), 1):
            if isinstance(nutrition_cache.get(ident), dict) or row.get("canonicalEnglishNeedsReview"):
                continue
            query = _text(row.get("canonicalName"))
            print(f"[nutrition] {index}/{len(ingredients)} {query}", flush=True)
            nutrition_cache[ident] = _lookup_fdc(query, fdc_key) or {"unresolved": True}
            _save_json(nutrition_path, nutrition_cache)
            if args.fdc_delay:
                time.sleep(args.fdc_delay)

    nutrition = {
        ident: row for ident, row in nutrition_cache.items()
        ...
        for item in variant.get("ingredients") or []:
            ...
            compact.append({
                key: value for key, value in {
                    "key": _text(item.get("foodKey") or item.get("key")),
                    "canonicalName": _text(global_row.get("canonicalName")) or _text(item.get("foodName") or item.get("name")),
                    "translations": deepcopy(global_row.get("translations") or {}),
                    "quantity": item.get("quantity"), "unit": item.get("unit"), "unitKey": item.get("unitKey"),
                }.items() if value not in ("", None, {})
            })
        variant["ingredients"] = compact
        variant["nutrition"] = _recipe_nutrition(compact, nutrition, variant.get("servings"))
    ...
    for variant in variants:
        ...
        group = groups.setdefault(key, {"groupingFunctionalId": grouping, "canonicalName": "", "variants": []})
        group["variants"].append({k: deepcopy(v) for k, v in variant.items() if v not in (None, "", [], {})})
        if _text(variant.get("language")).lower() == "en" and _text(variant.get("title")):
    ...
    for key, group in groups.items():
        if group["canonicalName"]:
            continue
        override = _text((recipe_overrides or {}).get(key)) if isinstance(recipe_overrides, dict) else ""
        if override:
            group["canonicalName"] = override
        else:
            group["canonicalName"] = next((_text(v.get("title")) for v in group["variants"] if _text(v.get("title"))), "")
            group["canonicalEnglishNeedsReview"] = True

    unresolved_ingredients = sum(bool(row.get("canonicalEnglishNeedsReview")) for row in ingredients.values())
    unresolved_recipes = sum(bool(row.get("canonicalEnglishNeedsReview")) for row in groups.values())
    failed_details = sum(int(row["failedDetails"]) for row in stats)
    all_hydrated = all(row["hydratedVariants"] == row["uniqueVariants"] for row in stats)
    nutrition_complete = len(nutrition) == len(ingredients)
    complete = bool(
        not unresolved_ingredients and not unresolved_recipes and failed_details == 0
        and all_hydrated and (args.allow_missing_nutrition or nutrition_complete)
    )
    payload = {
        "schemaVersion": 1,
        ...
        "source": {
            ...
            "applianceGroup": "APPLIANCE_GROUP_15", "recipeType": "BRAND",
            "sourceCatalogCount": len(recipe_languages.SUPPORTED_RECIPE_LANGUAGES),
            "catalogs": stats, "failedDetailCount": failed_details,
        ...
        "ingredients": sorted(
            ({k: v for k, v in row.items() if v not in ("", None, {}, [])} for row in ingredients.values()),
            key=lambda row: _text(row.get("canonicalName")).casefold(),
        ),
        "recipes": sorted(
            ({k: v for k, v in row.items() if v not in ("", None, {}, [])} for row in groups.values()),
            key=lambda row: _text(row.get("canonicalName")).casefold(),
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(COMPONENT / "catalog" / "merged_catalog.v1.json"))
    parser.add_argument("--nutrition-cache", default=str(ROOT / ".catalog-build" / "fdc-nutrition-cache.json"))
    ...
    payload = build(args)
    print(json.dumps({
        "catalogVersion": payload["catalogVersion"], "complete": payload["complete"],
        "recipes": len(payload["recipes"]), "ingredients": len(payload["ingredients"]),
        "failedDetails": payload["source"]["failedDetailCount"],
        "nutritionResolvedCount": payload["source"]["nutritionResolvedCount"],
        "output": args.output,
