...
print(f"PARTIAL_CATALOGS={sum(r['state']=='PARTIAL' for r in cs)}")
print(f"TRUNCATED_CATALOGS={sum(r['state']=='TRUNCATED' for r in cs)}")
print(f"ERROR_CATALOGS={sum(r['state']=='ERROR' for r in cs)}")
print(f"UNPARSED_ROWS={sum(int(r.get('unparsedRows') or 0) for r in cs)}")
print(f"MISSING_NAME_ROWS={sum(int(r.get('missingNameRows') or 0) for r in cs)}")
