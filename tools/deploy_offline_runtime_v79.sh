#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v79.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=c7096d7e3824cf1a998e07fffe95dff057d29c51
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v79-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v79 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v79-bundle.js ]] || die 'Source is missing v79 runtime files.'
...
printf 'Waiting for Home Assistant and the v79 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.4/cook4me-panel-v79-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v79-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v79 panel within the startup timeout.'
