#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v66.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=76dbeaa5d113e402448abdeb979ef1076246e561
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v66-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v66-bundle.js ]] || die 'Source is missing v66 runtime files.'
...
printf 'Waiting for Home Assistant and the v66 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.15.8/cook4me-panel-v66-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v66-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v66 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app) and try a dish or ingredient in Greek; then Today with all meal types\n'
