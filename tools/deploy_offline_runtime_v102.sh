#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v102.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=2f9d326a4c4f0feda3231e08f00335e8c7657d6d
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v102-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 147),
        ("price_portions.v1.json", "portions", 122),
    ...
    assert count == 518, f"Merged offline price snapshot: expected 518 observations, found {count}"
    ...
    added = [row for row in evidence['observations'] if 'v102-20260917' in str(row['id'])]
    assert len(added) == 4, "v102 package references missing"
    for category, unit in (("en:coconut-creams", "ml"), ("cook4me:dried-split-peas", "g"),
                           ("en:shiitake-mushrooms", "g"), ("en:radishes", "g")):
        assert prices.snapshot_observations(category=category, country="DE", currency="EUR", unit=unit), f"{category}/{unit}: v102 reference missing"
    for name, quantity, unit, expected in (("Shiitake mushroom", 2, "pcs", 38), ("Coconut cream", 1, "tbsp", 15)):
        options = measurements.price_options({"name": name, "quantity": quantity, "unit": unit})
        assert any(row["unit"] == "g" and abs(row["quantity"] - expected) < .001 for row in options), f"{name}: v102 USDA portion missing"
...
printf 'Fetching tested Cook4me v102 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py && -s $STAGED/price_allowances.py ]] || die 'Source is missing v102 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v102-bundle.js ]] || die 'Source is missing v102 runtime files.'
...
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_expansion_v102.py -v
...
printf 'Waiting for Home Assistant and the v102 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.17.6/cook4me-panel-v102-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v102-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v102 panel within the startup timeout.'
