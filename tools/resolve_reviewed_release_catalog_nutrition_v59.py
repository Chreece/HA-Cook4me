#!/usr/bin/env python3
"""Resolve catalog nutrition only from explicitly reviewed USDA FDC identities.

No search result is accepted automatically. A review file must bind each Cook4Me
ingredient identity to an exact USDA FDC ID. The resolver fetches that exact
record, verifies the ID, extracts generic per-100-g nutrients, and persists no
API credentials.
"""
...
def load_reviews(root: Path = TOOLS) -> dict[str, dict[str, Any]]:
    ...
    for path in _review_paths(root):
        value = _load(path, {})
        if not isinstance(value, dict) or value.get("kind") != "cook4me-reviewed-nutrition-source":
        ...
        for raw in value.get("items") or []:
            ...
            if existing:
                if int(existing["fdcId"]) != fdc_id:
                    raise RuntimeError(
                        f"conflicting nutrition review for {ingredient_id}: "
                        f"{existing['fdcId']} vs {fdc_id}"
                    )
                continue
            out[ingredient_id] = normalized
    return out


def _nutrient(food: dict[str, Any], tokens: tuple[str, ...], unit: str | None = None) -> float | None:
...
def fdc_profile(food: dict[str, Any]) -> dict[str, Any] | None:
    values = {
        ...
        "carbohydrates": _nutrient(food, ("carbohydrate by difference", "carbohydrate")),
        "sugars": _nutrient(food, ("total sugars", "sugars total")),
        "fat": _nutrient(food, ("total lipid fat", "total fat")),
        "saturatedFat": _nutrient(food, ("fatty acids total saturated", "saturated fat")),
...
def fetch_fdc_detail(fdc_id: int, api_key: str) -> dict[str, Any]:
    url = FDC_DETAIL_URL + urllib.parse.quote(str(fdc_id)) + "?api_key=" + urllib.parse.quote(api_key)
    request = urllib.request.Request(
        url,
        headers={"Accept": "application/json", "User-Agent": "HA-Cook4me-reviewed-nutrition/1"},
    )
...
def resolve(
    ...
    *,
    ...
    if queue.get("kind") != "cook4me-release-catalog-nutrition-queue-v59":
        raise RuntimeError("expected cook4me-release-catalog-nutrition-queue-v59")
    output = dict(cache)
    ...
    for raw in queue.get("tasks") or []:
        ...
        if returned_id != fdc_id:
            raise RuntimeError(f"FDC identity mismatch: requested {fdc_id}, got {returned_id}")
    ...
    summary = {
        "queueTaskCount": len([row for row in queue.get("tasks") or [] if isinstance(row, dict)]),
...
def main() -> int:
    ...
    if not api_key:
        raise SystemExit("reviewed nutrition resolution requires --fdc-key or FDC_API_KEY")
