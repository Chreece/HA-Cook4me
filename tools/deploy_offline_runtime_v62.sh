#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v62.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=66d813be424447c42bbdd00168264e5f1264d4de
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v62-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v62-bundle.js ]] || die 'Source is missing v62 runtime files.'
...
printf 'Preparing ingredient names for your UI language before stopping Home Assistant...\n'
python3 "$SOURCE/tools/build_ingredient_ui_locale.py" \
    --language "${COOK4ME_UI_LANGUAGE:-el}" \
    --output "$STAGED/catalog_ui_locales/${COOK4ME_UI_LANGUAGE:-el}.json" \
    --cache "$CONFIG/.storage/cook4me_ui_locales/${COOK4ME_UI_LANGUAGE:-el}.json" \
    --base-url "${COOK4ME_OLLAMA_URL:-http://127.0.0.1:11434}" \
...
printf 'Waiting for Home Assistant and the v62 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.15.4/cook4me-panel-v62-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v62-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v62 panel within the startup timeout.'
