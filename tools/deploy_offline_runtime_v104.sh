#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v104.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=8ac03804b7b28124af13e428d208f4feaa0550a9
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v104-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    assert len(added) == 2, "v104 package references missing"
    for category in ("en:misos", "cook4me:fresh-shimeji"):
        assert prices.snapshot_observations(category=category, country="DE", currency="EUR", unit="g"), f"{category}: v104 reference missing"
...
printf 'Fetching tested Cook4me v104 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py && -s $STAGED/price_allowances.py ]] || die 'Source is missing v104 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v104-bundle.js ]] || die 'Source is missing v104 runtime files.'
...
printf 'Waiting for Home Assistant and the v104 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.17.8/cook4me-panel-v104-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v104-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v104 panel within the startup timeout.'
...
printf 'Reopen Cook4Me. Prices use amounts and icons. Fullscreen refresh sits beside the total. Ingredient details open above the recipe. Integration-disabled device telemetry is restored; user-disabled entities stay disabled.\n'
