#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v81.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=feaf043f429b812221db4d2c5a5033519b5f9c11
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v81-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v81 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v81-bundle.js ]] || die 'Source is missing v81 runtime files.'
...
printf 'Waiting for Home Assistant and the v81 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.6/cook4me-panel-v81-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v81-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v81 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app). In Your kitchen, use Scan a product or Add manually. Manage storage places separately and select an image-capable AI Task under Integration settings for photo recognition.\n'
