#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v85.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=7fb8a9558c9ce5f4c25139cde6a3f5d6f2ddcdb5
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v85-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v85 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v85 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v84-bundle.js ]] || die 'Source is missing v84 runtime files.'
...
printf 'Waiting for Home Assistant and the v84 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.9/cook4me-panel-v84-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v84-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v84 panel within the startup timeout.'
