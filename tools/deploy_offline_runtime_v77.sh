#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v77.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=5cf5e00c55254c9d8260d6b6de5a9600afa8dc61
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v77-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v77 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v77-bundle.js ]] || die 'Source is missing v77 runtime files.'
...
printf 'Waiting for Home Assistant and the v77 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.2/cook4me-panel-v77-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v77-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v77 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app) after installing the v77 sending with complete ingredient replacements. Refresh Today, Week and Official recipes for the selected diet.\n'
