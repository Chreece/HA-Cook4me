#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v90.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=55571e2c0626d5d08f0f0e5883760118b4410cb0
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v90-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline catalog opened network")):
    ...
    for filename, key, expected in (
        ...
        ("price_portions.v1.json", "portions", 87),
...
printf 'Fetching tested Cook4me v90 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v90 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v90-bundle.js ]] || die 'Source is missing v90 runtime files.'
...
printf 'Waiting for Home Assistant and the v90 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.14/cook4me-panel-v90-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v90-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v90 panel within the startup timeout.'
