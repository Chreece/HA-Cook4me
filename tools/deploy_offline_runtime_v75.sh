#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v75.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=172f8bb16aea5b5b0a641d878314b4e6f6d5e3f8
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v75-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v75 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v74-bundle.js ]] || die 'Source is missing v74 runtime files.'
...
printf 'Waiting for Home Assistant and the v74 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.15.16/cook4me-panel-v74-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v74-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v74 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app) after installing the v75 queued-send, persistence and announcement fixes.\n'
