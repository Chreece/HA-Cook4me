#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v96.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=8ca9a66792b4ec90e42ec5bf8007f2e3c7b07358
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v96-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 133),
        ("price_portions.v1.json", "portions", 116),
    ...
    assert count == 504, f"Merged offline price snapshot: expected 504 observations, found {count}"
...
printf 'Fetching tested Cook4me v96 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v96 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v96-bundle.js ]] || die 'Source is missing v96 runtime files.'
...
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_expansion_v96.py -v
...
printf 'Waiting for Home Assistant and the v96 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.20/cook4me-panel-v96-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v96-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v96 panel within the startup timeout.'
