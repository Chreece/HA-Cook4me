#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v88.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=410cb3ed310f0351c9b260831911967996b3abd9
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v88-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline catalog opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 82),
        ("price_portions.v1.json", "portions", 69),
    ...
    assert count == 453, f"Merged offline price snapshot: expected 453 observations, found {count}"
...
printf 'Fetching tested Cook4me v88 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v88 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v88-bundle.js ]] || die 'Source is missing v88 runtime files.'
...
printf 'Waiting for Home Assistant and the v88 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.12/cook4me-panel-v88-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v88-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v88 panel within the startup timeout.'
