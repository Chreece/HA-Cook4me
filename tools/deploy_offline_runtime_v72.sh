#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v72.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=ffc498b0b44eff28fd4d19f5f533ba61e072d59e
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v72-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v72-bundle.js ]] || die 'Source is missing v72 runtime files.'
...
printf 'Waiting for Home Assistant and the v72 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.15.14/cook4me-panel-v72-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v72-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v72 panel within the startup timeout.'
