#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v84.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=d88f73d0420b48bea075986b20d15a3cf274a8fd
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v84-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v84 commit %s...\n' "$SOURCE_COMMIT"
...
printf 'Reopen Cook4Me (reload the browser/app). Set up household and member diet profiles in My kitchen & preferences. Use the first recipe-filter button to select Manual, Whole household or a member. Manual filter edits keep their values and switch the source to Manual.\n'
