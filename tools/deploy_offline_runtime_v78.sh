#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v78.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=0b0e3fcd0c8fd00b38d6b8fd5a0b63b6fb2f2138
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v78-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v78 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v78-bundle.js ]] || die 'Source is missing v78 runtime files.'
...
printf 'Waiting for Home Assistant and the v78 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.3/cook4me-panel-v78-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v78-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v78 panel within the startup timeout.'
