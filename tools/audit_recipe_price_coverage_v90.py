#!/usr/bin/env python3
"""Compare actual offline eligibility against v89, including both snapshot files.
...
BASE = '9346ea913a7b1c68a5888161b0a277803f1fd9a3'
EXAMPLES = {'287823','307808','826311','816820','862912','252627','834656','331197','317075','487446','357993','734674','341615','834652'}
        ...
        for row in json.loads((path / 'catalog' / filename).read_text())['observations']:
            if row['country'] == 'DE' and row['currency'] == 'EUR' and row.get('usable') and row.get('basisQuantity') \
                ...
                for tag in row['categories']:
                    if tag not in row.get('categoryExclusions', []): references[tag].append(row)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for filename in ['automatic_prices.py', 'price_measurements.py', 'price_units.py', 'inventory.py',
                         'price_identity.py', 'catalog/price_reference_portions.v1.json', 'catalog/price_densities.v1.json', 'catalog/price_portions.v1.json', 'catalog/observed_prices.v1.json', 'catalog/retail_prices.v1.json']:
        ...
        result = {'market': 'DE', 'currency': 'EUR', 'asOf': args.as_of, 'base': args.base, 'variantCounts': True,
                  'results': {label: audit(path, '_audit90_' + label, catalog, date.fromisoformat(args.as_of))
