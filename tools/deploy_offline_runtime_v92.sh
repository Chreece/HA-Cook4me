#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v92.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=242f6f339fcbfad922871a8ca484cb4eb47f5330
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v92-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 101),
    ...
    assert count == 472, f"Merged offline price snapshot: expected 472 observations, found {count}"
    ...
    assert prices.snapshot_observations(category="en:green-asparagus", country="DE", currency="EUR", unit="g"), "Green asparagus price missing"
    assert any(row["unit"] == "ml" and row["quantity"] == 15 for row in measurements.price_options({"name": "Olive oil", "quantity": 1, "unit": "EL"})), "Localized tablespoon conversion missing"
    benchmarks = importlib.import_module(f"{package.__name__}.price_benchmarks")
    estimate = benchmarks.fallback_estimate({"name": "Radish", "quantity": 70, "unit": "g", "priceCatalogMatched": True}, country="DE", currency="EUR")
...
printf 'Fetching tested Cook4me v92 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v92 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v92-bundle.js ]] || die 'Source is missing v92 runtime files.'
...
printf 'Running the pricing and cache checks for this build...\n'
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_optimization_v92.py -v
...
printf 'Waiting for Home Assistant and the v92 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.16/cook4me-panel-v92-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v92-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v92 panel within the startup timeout.'
