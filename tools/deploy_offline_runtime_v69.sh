#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v69.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=e6fd14c71546367224dffca5a0515c1e37f268c2
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v69-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v69-bundle.js ]] || die 'Source is missing v69 runtime files.'
...
printf 'Waiting for Home Assistant and the v69 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.15.11/cook4me-panel-v69-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v69-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v69 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app) for titles above photos and photo action buttons.\n'
printf 'Generate the next seven days once to replace duplicate recipes in your saved weekly plan.\n'
