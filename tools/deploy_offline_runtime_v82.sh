#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v82.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=9b0aa5fce6734f71a9304cd72d64e9e0f3ae94be
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v82-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v82 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v82-bundle.js ]] || die 'Source is missing v82 runtime files.'
...
printf 'Waiting for Home Assistant and the v82 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.7/cook4me-panel-v82-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v82-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v82 panel within the startup timeout.'
...
printf 'Reopen Cook4Me (reload the browser/app). Use the seven navigation icons. Recipe creator contains manual and AI sections; My kitchen & preferences contains Add product. Use the cash-sync icon in the top bar to update prices for the recipes shown.\n'
