#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v93.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=686c3fb1a8470d3ac6349eb0e38305930c69c90d
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v93-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v93 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v93 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v93-bundle.js ]] || die 'Source is missing v93 runtime files.'
...
printf 'Running the cancellation checks for this build...\n'
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_jobs_v93.py -v
...
printf 'Waiting for Home Assistant and the v93 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.17/cook4me-panel-v93-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v93-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v93 panel within the startup timeout.'
