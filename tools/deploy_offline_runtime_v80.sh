#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v80.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=7cb8f74f786f112e9c67fd109f1d83102a1424c5
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v80-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
printf 'Fetching tested Cook4me v80 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v80-bundle.js ]] || die 'Source is missing v80 runtime files.'
...
printf 'Waiting for Home Assistant and the v80 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.5/cook4me-panel-v80-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v80-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v80 panel within the startup timeout.'
