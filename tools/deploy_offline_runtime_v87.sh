#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v87.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=439b8453144fa59de42b0230a47edf4908a0d167
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v87-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline catalog opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 34),
    ...
    assert count == 405, f"Merged offline price snapshot: expected 405 observations, found {count}"
...
printf 'Fetching tested Cook4me v87 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v87 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v87-bundle.js ]] || die 'Source is missing v87 runtime files.'
...
printf 'Waiting for Home Assistant and the v87 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.11/cook4me-panel-v87-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v87-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v87 panel within the startup timeout.'
