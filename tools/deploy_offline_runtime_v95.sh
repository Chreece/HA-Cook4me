#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v95.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=354accf184f12ae387e8f191b8153bbeb9da82a0
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v95-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline prices opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 122),
        ("price_portions.v1.json", "portions", 109),
    ...
    assert count == 493, f"Merged offline price snapshot: expected 493 observations, found {count}"
    measurements = importlib.import_module(f"{package.__name__}.price_measurements")
    for name, quantity, unit, expected in (("Fennel", 1, "", 234), ("Cumin", 1, "tsp", 2),
                                            ("Sesame seed", 2, "tbsp", 18), ("Basil", 5, "leaf", 2.5)):
    ...
    for category in ("en:fennel", "en:sesame", "en:tahini", "en:powdered-sugars", "cook4me:baking-soda"):
        assert prices.snapshot_observations(category=category, country="DE", currency="EUR", unit="g"), f"{category}: reference missing"
...
printf 'Fetching tested Cook4me v95 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v95 price evidence.'
[[ -s $STAGED/websocket_v36.py && -s $STAGED/job_runtime.py && -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v95-bundle.js ]] || die 'Source is missing v95 runtime files.'
...
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_expansion_v95.py -v
...
printf 'Waiting for Home Assistant and the v95 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.19/cook4me-panel-v95-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v95-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v95 panel within the startup timeout.'
