#!/usr/bin/env bash
# Run as a standalone script: sudo bash deploy_offline_runtime_v91.sh
# Do not source this file into an interactive shell.
set -Eeuo pipefail

SOURCE_COMMIT=1d99fec620cbea433841f9c8a4e23e9539b31b10
...
RUN=$(mktemp -d "$CONFIG/.cook4me-deploy/v91-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")
SOURCE=$RUN/source
...
with patch.object(socket, "socket", side_effect=AssertionError("offline catalog opened network")):
    ...
    for filename, key, expected in (
        ...
        ("retail_prices.v1.json", "observations", 100),
        ("price_portions.v1.json", "portions", 88),
        ("price_reference_portions.v1.json", "portions", 3),
        ("price_densities.v1.json", "densities", 1),
    ...
    assert count == 471, f"Merged offline price snapshot: expected 471 observations, found {count}"
    measurements = importlib.import_module(f"{package.__name__}.price_measurements")
    options = measurements.price_options({"name": "Thick crème fraîche", "quantity": 300, "unit": "ml"})
    assert any(row["unit"] == "g" and abs(row["quantity"] - 293.4) < 0.001 for row in options), "Sourced density conversion failed"
    assert prices.snapshot_observations(category="cook4me:vegetable-stock", country="DE", currency="EUR", unit="ml"), "Prepared stock price missing"
    assert prices.snapshot_observations(category="cook4me:almond-drink", country="DE", currency="EUR", unit="ml"), "Almond drink price missing"
    assert any(row["unit"] == "g" and row["quantity"] == 118 for row in measurements.price_options({"name": "Banana", "quantity": 1})), "Banana count conversion missing"
    assert any(row["unit"] == "g" and row["quantity"] == 7.5 for row in measurements.price_options({"name": "Curry paste", "quantity": .5, "unit": "tbsp"})), "Curry paste spoon conversion missing"
    benchmarks = importlib.import_module(f"{package.__name__}.price_benchmarks")
    estimate = benchmarks.fallback_estimate({"name": "Green asparagus", "quantity": 70, "unit": "g", "priceCatalogMatched": True}, country="DE", currency="EUR")
...
printf 'Fetching tested Cook4me v91 commit %s...\n' "$SOURCE_COMMIT"
...
[[ -s $STAGED/catalog/observed_prices.v1.json && -s $STAGED/catalog/price_portions.v1.json && -s $STAGED/catalog/retail_prices.v1.json && -s $STAGED/catalog/price_densities.v1.json && -s $STAGED/catalog/price_reference_portions.v1.json && -s $STAGED/catalog/price_benchmarks.v1.json && -s $STAGED/price_benchmarks.py && -s $STAGED/price_snapshot.py ]] || die 'Source is missing v91 price evidence.'
[[ -s $STAGED/query_vocabulary.json && -s $STAGED/frontend/cook4me-panel-v91-bundle.js ]] || die 'Source is missing v91 runtime files.'
...
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_runtime_audit_v74.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_delivery_audit_v75.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_diet_substitutions_v76.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_diet_send_v77.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_product_capture_v78.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_automatic_prices_v79.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_ingredient_endpoint_v80.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_device_state_v81.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_refresh_v82.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_diet_profiles_v83.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_coverage_v84.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_offline_catalog_v84.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_offline_prices_v85.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_recipe_cost_coverage_v86.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_gaps_v87.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_expansion_v88.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_gaps_v89.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_gaps_v90.py -v
docker exec --user 0 --workdir "$CONTAINER_SOURCE" "$CONTAINER" python -B -m unittest discover -s tests -p test_price_fallback_v91.py -v
...
printf 'Waiting for Home Assistant and the v91 Recipe Hub panel...\n'
deadline=$((SECONDS + HEALTH_SECONDS))
panel_ready=0
while (( SECONDS < deadline )); do
    if curl --fail --silent --max-time 5 "${HA_URL%/}/" >/dev/null &&
       curl --fail --silent --max-time 5 "${HA_URL%/}/cook4me_static/2026.9.16.15/cook4me-panel-v91-bundle.js" -o "$RUN/served-panel.js" &&
       cmp -s "$RUN/served-panel.js" "$TARGET/frontend/cook4me-panel-v91-bundle.js"; then
        panel_ready=1
        break
    fi
    sleep 2
done
(( panel_ready )) || die 'Home Assistant did not serve the installed v91 panel within the startup timeout.'
...
printf 'Reopen Cook4Me. Verified offline price estimates are ready for supported ingredients in your shopping country. Use Update recipe prices to request fresh observations.\n'
