#!/usr/bin/env python3
"""Compare reference, rough-budget and zero-allowance coverage with v103."""
import argparse
...
BASE = 'f86cdec36dd8a08a713169c4f1bc62afe7e14d9d'
...
if __name__ == '__main__':
    ...
    if args.before_json:
        ...
        with tempfile.TemporaryDirectory(prefix='cook4me-prices-v104-') as directory:
            archive = subprocess.check_output(['git', 'archive', BASE, 'custom_components/cook4me'], cwd=COMPONENT.parents[1])
            with tarfile.open(fileobj=io.BytesIO(archive)) as tar:
                tar.extractall(directory, filter='data')
            before = audit(Path(directory) / 'custom_components/cook4me', '_prices104_before')
    after = audit(COMPONENT, '_prices104_after')
