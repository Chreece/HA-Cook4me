...
with sync_playwright() as p:
    ...
    for width,height in sizes:
        ...
        page.evaluate("""(()=>{app.shadowRoot.querySelector('[data-v218-scale-probe]')?.remove();app.shadowRoot.querySelector('[data-v218-nutrition-probe]')?.remove();app._nutritionSettings={fdcApiKeyConfigured:false,fdcMode:'demo',catalogCount:4,catalogTotal:6292,remaining:6288,blockedFailures:1,actionableRemaining:6287,genericFallbackCatalog:true,unresolvedDetails:[{identity:'k:rice',name:'Rice',query:'rice',reason:'ambiguous',confidence:.9,retryAfter:'2026-10-24T08:00:00+00:00'}]};const probe=document.createElement('section');probe.dataset.v218NutritionProbe='';const html=app._nutritionSettingsHtml();if(!html.includes('data-v218-unresolved'))throw new Error('v218 nutrition review HTML missing despite explicit unresolved row');probe.innerHTML=html;app.shadowRoot.append(probe);app._bindNutritionSettings(probe);})()""")
        page.wait_for_function("app.shadowRoot.querySelector('[data-v218-nutrition-probe] [data-v218-unresolved]')")
        check(page,"app.shadowRoot.querySelectorAll('[data-v218-nutrition-probe] [data-v218-unresolved]').length===1&&app.shadowRoot.querySelector('[data-v218-nutrition-probe] .v218-fallback-help')?.textContent.includes('προαιρετικός')",prefix+'nutrition fallback explains unresolved entries instead of implying broken catalog')
        page.locator('[data-v218-nutrition-probe] .v218-nutrition-review > summary').click()
        page.locator('[data-v218-nutrition-probe] [data-v218-unresolved] > summary').click()
        page.locator('[data-v218-nutrition-probe] [data-v218-unresolved] [data-v218-retry]').click()
        page.wait_for_timeout(80)
        check(page,"app._nutritionSettings.blockedFailures===0&&app._nutritionSettings.unresolvedDetails.length===0",prefix+'unresolved nutrition can be retried immediately')
        page.evaluate("app.shadowRoot.querySelector('[data-v218-nutrition-probe]')?.remove()")
