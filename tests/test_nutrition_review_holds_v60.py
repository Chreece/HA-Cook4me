...
class NutritionReviewHoldTests(unittest.TestCase):
    ...
    def test_twelve_holds_pin_original_bindings_and_merged_member_ids(self):
        self.assertEqual(len(self.rows), 12)
        # The reconciled semantic corpus includes one more exact source-local alias.
        # It must inherit the hold, never escape it through its alternate spelling.
        members = [m for r in self.rows for m in r["memberIngredientIds"]]
        self.assertEqual(len(members), 23)
        self.assertEqual(len(set(members)), 23)
        self.assertEqual(sum(m.startswith("local:") for m in members), 22)
    ...
    def test_target_resolver_keeps_all_held_targets_pending_without_fetching(self):
        ...
        self.assertEqual(report["summary"]["heldIdentities"], 23)
        self.assertEqual(report["summary"]["pendingReviewTargetCount"], 12)
        self.assertEqual(report["summary"]["pendingIdentityCount"], 23)
        self.assertEqual(report["summary"]["rejectedHeldCacheCount"], 23)
    ...
    def test_exact_identity_resolver_cannot_bypass_target_holds(self):
        ...
        self.assertEqual(output, {})
        self.assertEqual(report["summary"]["heldIdentities"], 23)
        self.assertEqual(report["summary"]["pendingCount"], 23)
