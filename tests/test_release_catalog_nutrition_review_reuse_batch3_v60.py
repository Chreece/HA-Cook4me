...
REUSE_FILE = TOOLS / "release_catalog_reviewed_nutrition_targets_012.v1.json"
EXPECTED_DIGEST = "1242116c22b442de1afd0d7c3bfc0ca5d0555ca9cd336bcac60fe9b582196ade"
...
PRIOR_FILES = (
    ...
    TOOLS / "release_catalog_reviewed_nutrition_targets_010.v1.json",
...
class NutritionReviewedDecisionReuseBatch3V60Tests(unittest.TestCase):
    def test_reuse_artifact_is_locked(self):
        ...
        self.assertEqual(len({row["reviewTargetId"] for row in items}), 3)
        self.assertEqual(_digest(items), EXPECTED_DIGEST)
        self.assertEqual(sum(int(row["usageCountAtReview"]) for row in items), 32)
        self.assertEqual(sum(row["fdcDataType"] == "Survey (FNDDS)" for row in items), 1)
        self.assertEqual(sum(row["fdcDataType"] == "SR Legacy" for row in items), 2)
        self.assertTrue(all(int(row["candidateEvidenceRank"]) == 1 for row in items))
    ...
    def test_every_reuse_has_one_unambiguous_prior_exact_canonical_decision(self):
        ...
        self.assertEqual(len(prior_rows), 580)
