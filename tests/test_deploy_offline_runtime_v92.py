...
SCRIPT = Path(__file__).resolve().parents[1] / "tools/deploy_offline_runtime_v92.sh"
COMMIT = "242f6f339fcbfad922871a8ca484cb4eb47f5330"
...
if name == "git":
    if args[0] == "init":
        ...
        (staged / "frontend/cook4me-panel-v92-bundle.js").write_text("new panel\n")
        ...
        print("242f6f339fcbfad922871a8ca484cb4eb47f5330")
elif name == "docker":
    if args[0] == "inspect":
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_success_keeps_backup_and_installs_pinned_complete_component(self):
        ...
        runs = list((root / "config/.cook4me-deploy").glob("v92-*"))
        ...
        self.assertEqual(result.stdout.count("Offline price snapshot: 472 observations"), 2)
        ...
        self.assertEqual(suites, ["test_price_optimization_v92.py", "test_price_fallback_v91.py"])
    ...
    def test_missing_offline_price_file_leaves_installation_untouched(self):
        for scenario in ("missing_prices", "missing_retail", "missing_density", "missing_reference_portions", "missing_benchmarks"):
