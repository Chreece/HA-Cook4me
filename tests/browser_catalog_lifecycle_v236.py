"""Real Greek/German catalog choices and fermented-package opening controls."""
from datetime import datetime, timezone
from pathlib import Path
import shutil

from playwright.sync_api import expect, sync_playwright
from test_catalog_lifecycle_v236 import PACKAGES, SEASONS, actual_choice, catalog, opening
...
def run():
    ids = {r[1] for r in PACKAGES} | {r[0] for r in SEASONS} | {'local:hr:9644688c28222d3e4cad'}
    catalogs = {}
    for lang in ('el', 'de'):
    ...
    with sync_playwright() as p:
        ...
        for width, height in ((390, 844), (1440, 980)):
            ...
            for month in (2, 3, 4, 5, 8, 9, 11, 12):
            ...
            expect(section.locator('option').filter(has_text='Σέλινο')).to_contain_text('Stangensellerie')
            expect(section.locator('option').filter(has_text='Ρίζα μαϊντανού')).to_contain_text('Petersilienwurzel')
            page.clock.set_system_time(datetime(2026, 9, 25, 12, tzinfo=timezone.utc))
            for _, ident, code, rule, days in PACKAGES:
                ...
                page.evaluate("""async ({ingredient,code}) => {
                    ...
                    Object.assign(app._v78Draft,{brand:'dmBio',barcode:code,productName:ingredient.name,
                        quantity:240,unit:'g',ingredient,ingredientLinks:[ingredient],editorOpen:true,bestBefore:'2099-12-31'});
                ...
                expect(page.locator('main [data-draft="useWithinDays"]')).to_have_value(str(days))
            ...
            print(f'PASS {width}: three seasons, Greek/German supermarket names and two confirmed opening rules')
            page.close()
        browser.close()
