...
try{
 ...
 await page.goto('http://cook4me.test/');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v90-bundle.js'});
 await page.evaluate(()=>{
  globalThis.customElements.define('cook4me-v90-test',class extends customElements.get('cook4me-recipe-hub-panel-v90'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v90-test');
 ...
 assert.match(await page.locator('cook4me-v90-test [data-v82-card-cost]').first().innerText(),/0\.22/);
 assert.match(await page.locator('cook4me-v90-test [data-v82-card-cost]').first().innerText(),/1\/1 ingredients priced/);
 await page.evaluate(()=>{
 ...
 assert.match(await page.locator('cook4me-v90-test [data-v82-card-cost]').first().innerText(),/Subtotal.*2.12.*1\/2 ingredients priced/);
 assert.match(await page.locator('cook4me-v90-test [data-v82-card-cost]').first().getAttribute('title'),/Salt <unsafe>.*quantity missing/);
 assert.equal(await page.locator('cook4me-v90-test unsafe').count(),0);
 ...
 const panel=page.locator('cook4me-v90-test');
 ...
 const installHarness=()=>{
  globalThis.customElements.define('cook4me-cache-test',class extends customElements.get('cook4me-recipe-hub-panel-v90'){connectedCallback(){}});
 ...
 await page.reload();await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v90-bundle.js'});await page.evaluate(installHarness);
 ...
 for(const changes of [{testToken:'token-two'},{testUser:'bob'}]){
  await page.reload();await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v90-bundle.js'});await page.evaluate(changes=>Object.assign(window,changes),changes);await page.evaluate(installHarness);
 ...
 assert.deepEqual(errors,[]);console.log('v90 browser: offline previews, concurrency, entry isolation, persistent reload cache, changed evidence, retries, quantity sources and missing-cost explanation passed');
