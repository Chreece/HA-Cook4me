...
try{
 ...
 await page.goto('http://cook4me.test/');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v86-bundle.js'});
 await page.evaluate(()=>{
  globalThis.customElements.define('cook4me-v86-test',class extends customElements.get('cook4me-recipe-hub-panel-v86'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v86-test');
 ...
 assert.match(await page.locator('cook4me-v86-test [data-v82-card-cost]').first().innerText(),/0\.22/);
 ...
 const panel=page.locator('cook4me-v86-test');
 ...
 assert.match(await page.evaluate(()=>panel._v79Reference({source:'retail_snapshot',productName:'dmBio sugar',location:'dm Germany',date:'2026-09-16',amount:2.45,currency:'EUR',basisQuantity:500,basisUnit:'g',sourceUrl:'https://www.dm.de/p/d/1454994/dmbio-vollrohrzucker'})),/data-v86-retail.*dmBio sugar/);
 assert.deepEqual(errors,[]);console.log('v86 browser: offline previews, concurrency, entry isolation, quantity sources and missing-cost explanation passed');
