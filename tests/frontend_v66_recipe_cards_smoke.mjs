...
await import("../custom_components/cook4me/frontend/cook4me-panel-v66-bundle.js");
const panel=document.createElement("cook4me-recipe-hub-panel-v66"),requests=[];
...
panel._requestSection=async()=>{};panel._loadRecipeNutrition=async()=>{};panel._loadOverview=async()=>{};panel._loadIngredientCatalog=async()=>{};
...
assert.equal(cards()[0].querySelector(".rx-v66-body"),null);assert.equal(cards()[0].querySelectorAll("button").length,2);
...
dialog.querySelector('[data-v66-action="cook"]').click();dialog=panel.shadowRoot.querySelector("[data-v66-fullscreen]");assert.equal(dialog.querySelector('[aria-current="step"]').dataset.v66Step,"0");
...
panel._weekState={weekStart:"2026-09-14",settings:{},slots:all.map((recipe,i)=>({id:`slot${i}`,date:`2026-09-${14+i%7}`,mealType:"dinner",recipe:structuredClone(recipe)})),leftovers:[]};panel._weekStateEntry="one";panel._tab="week";panel._renderTab();assert.equal(cards().length,8);content().querySelector("[data-load-more]").click();await tick();assert.equal(cards().length,16);
...
panel.disconnectedCallback();console.log("v66: pagination, collapsed cards, ingredients, selectors, queue target, fullscreen cooking and filters passed");process.exit(0);
