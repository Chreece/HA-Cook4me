...
const fixture=JSON.parse(readFileSync(root+'/docs/price-expansion-v102-preview.json','utf8'));
...
try{
 ...
 await page.goto('about:blank');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v102-bundle.js'});
 await page.evaluate(fixture=>{
  globalThis.customElements.define('cook4me-v102-test',class extends globalThis.customElements.get('cook4me-recipe-hub-panel-v102'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v102-test');p._hass={language:'el'};p._uiIngredientLanguage=()=>p._hass.language;p._langCode=()=>p._hass.language;
 ...
 const p=page.locator('cook4me-v102-test');
 for(const language of ['en','de','el']){
  ...
  for(const id of ['169242','170580'])assert.ok(evidenceLinks.some(url=>url.includes(id)),id);
  assert.match(await p.evaluate(node=>node.shadowRoot.textContent),/4[,.]40/);
  assert.equal(await page.evaluate(()=>state.cost.priceConfidence),'reference');
 }
 for(const host of ['fuchsgruppe.shop','www.davert.de','asianbrand.de','frische-kontor.de']){
 ...
 assert.deepEqual(errors,[]);console.log('v102: actual offline costs and four package references and USDA estimates render in English, German and Greek; reviewed source links validate correctly');
