"""Batch 27: real season filters, German root names and red-fruit opening UI."""
...
from test_catalog_lifecycle_v243 import JAM, CODE, RULE, NAMES, PANTRY, SEASONS, actual_choice, catalog, opening
...
def run():
    ids = set(SEASONS) | set(PANTRY) | {row[0] for row in NAMES}
    catalogs = {}
    for lang in ('el', 'de'):
    ...
    scenarios = []
    for ident, brand, barcode in (
        (JAM, 'Xucker', CODE), (JAM, 'Xucker', '4260248063892'),
        (JAM, 'Xucker', '4260248063908'), (JAM, 'dmBio', CODE),
        ('M_FOOD_462', 'dmBio', '4067796084849'),
        ('M_FOOD_362', 'dmBio', '4066447948417'),
    ):
        lot = {'brand': brand, 'barcode': barcode}
        scenarios.append({'ingredient': actual_choice(catalogs['el'], ident), 'lot': lot, 'rule': RULE,
        ...
        for width, height in ((390, 844), (1440, 980)):
            ...
            for country in ('DE', 'GR'):
                ...
                for month in range(1, 13):
                    ...
                    for ident, (_, months, source_country, _) in SEASONS.items():
                        expect(option(ident)).to_have_count(int(source_country != country or month in months))
                    for ident in PANTRY:
            ...
            expect(option('M_FOOD_328')).to_contain_text('Γογγύλι')
            expect(option('M_FOOD_433')).to_contain_text('Ρουταμπάγκα')
            ...
            for scenario in scenarios:
                page.evaluate("""async ({ingredient,lot,rules}) => {
                    ...
                    Object.assign(app._v78Draft,{...lot,productName:ingredient.name,quantity:220,unit:'g',
                ...
                if scenario['rules']:
                    ...
                    expect(page.locator('main [data-draft="useWithinDays"]')).to_have_value('10')
            ...
            print(f'PASS {width}: twelve-month DE/GR filters, turnip/swede names, exact red-fruit package, unknown syrup/spread windows and opening confirmation')
            page.close()
        browser.close()
