...
def lot_signature(lot):
    return (lot.get("quantity"), lot.get("bestBefore"))
...
class InventoryTests(unittest.TestCase):
    ...
    def test_legacy_single_date_stock_migrates_to_one_batch(self):
        stock = self.module.normalize_inventory([
            {"key": "M_FOOD_BUTTER", "name": "Butter", "quantity": 250, "unit": "g", "bestBefore": "2026-09-10"}
        ])
        self.assertEqual([lot_signature(x) for x in stock[0]["lots"]], [(250, "2026-09-10")])
    ...
    def test_unlimited_stock_never_gets_reduced(self):
        ...
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_WATER", "quantity": 2, "unit": "l", "consume": True}]
        )
    ...
    def test_best_before_is_stored_for_finite_and_unlimited_stock(self):
        finite = self.module.add_inventory_item(
            [], {"key": "M_FOOD_YOGURT", "name": "Yogurt"}, quantity=500, unit="g", best_before="2026-09-20"
        )
        unlimited = self.module.add_inventory_item(
            [], {"key": "M_FOOD_WATER", "name": "Water"}, unlimited=True, best_before="2027-01-01"
        )
        self.assertEqual(finite[0]["bestBefore"], "2026-09-20")
        self.assertEqual([lot_signature(x) for x in finite[0]["lots"]], [(500, "2026-09-20")])
    ...
    def test_finite_expiry_requires_amount(self):
        with self.assertRaisesRegex(ValueError, "attached to an amount"):
            self.module.add_inventory_item(
                [], {"key": "M_FOOD_YOGURT", "name": "Yogurt"}, best_before="2026-09-20"
            )

    def test_restock_preserves_amount_per_expiry_batch(self):
        stock = [{"key": "M_FOOD_BUTTER", "name": "Butter", "quantity": 250, "unit": "g", "bestBefore": "2026-10-15"}]
        updated = self.module.add_inventory_item(
            stock, {"key": "M_FOOD_BUTTER", "name": "Butter"}, quantity=250, unit="g", best_before="2026-09-30"
        )
        self.assertEqual(updated[0]["quantity"], 500)
        self.assertEqual(updated[0]["bestBefore"], "2026-09-30")
        self.assertEqual(
            [lot_signature(x) for x in updated[0]["lots"]],
            [(250, "2026-09-30"), (250, "2026-10-15")],
        )

    def test_best_before_can_be_edited_and_cleared(self):
        stock = [{"key": "M_FOOD_MILK", "name": "Milk", "quantity": 1, "unit": "l", "bestBefore": "2026-09-10"}]
        updated = self.module.update_inventory_item(
            stock, "k:M_FOOD_MILK", quantity=1, unit="l", best_before="2026-09-12"
        )
        ...
        cleared = self.module.update_inventory_item(
            updated, "k:M_FOOD_MILK", quantity=1, unit="l", best_before=""
        )
    ...
    def test_batch_list_can_be_edited_without_collapsing_dates(self):
        stock = [{"key": "M_FOOD_MILK", "name": "Milk", "unit": "l", "lots": [
            {"quantity": 1, "bestBefore": "2026-09-10"},
            {"quantity": 2, "bestBefore": "2026-09-20"},
        ]}]
        updated = self.module.update_inventory_item(
            stock, "k:M_FOOD_MILK", unit="l", lots=[
    ...
    def test_invalid_best_before_is_rejected_on_user_write(self):
        with self.assertRaises(ValueError):
            self.module.add_inventory_item(
                [], {"key": "M_FOOD_EGG", "name": "Egg"}, quantity=6, unit="pcs", best_before="31-12-2026"
            )
    ...
    def test_expiry_window_reports_only_the_expiring_batch_amount(self):
        stock = [
            ...
            {"key": "M_FOOD_TODAY", "name": "Today", "unit": "g", "lots": [
        ...
        rows = self.module.expiring_inventory_items(stock, today=date(2026, 9, 7), within_days=3, include_past=True)
    ...
    def test_opened_package_can_make_effective_expiry_earlier(self):
        stock = self.module.add_inventory_item(
            [], {"key": "M_FOOD_TOFU", "name": "Tofu"}, quantity=400, unit="g", best_before="2026-09-30",
            lot_metadata={"storage": "fridge", "openedAt": "2026-09-07", "useWithinDays": 3, "barcode": "12345678", "brand": "Test"},
        )
        lot = stock[0]["lots"][0]
        self.assertEqual(lot["effectiveBestBefore"], "2026-09-10")
        self.assertEqual(stock[0]["effectiveBestBefore"], "2026-09-10")
        rows = self.module.expiring_inventory_items(stock, today=date(2026, 9, 7), within_days=3)
        self.assertEqual(rows[0]["effectiveBestBefore"], "2026-09-10")
        self.assertTrue(rows[0]["openedDrivenExpiry"])
        self.assertEqual(rows[0]["storage"], "fridge")

    def test_opened_effective_expiry_controls_fefo_and_provenance_survives(self):
        stock = [{"key": "M_FOOD_TOFU", "name": "Tofu", "unit": "g", "lots": [
            {"id": "newer-printed", "quantity": 300, "bestBefore": "2026-09-20", "openedAt": "2026-09-07", "useWithinDays": 2, "barcode": "11111111", "productName": "Opened tofu", "storage": "fridge"},
            {"id": "older-printed", "quantity": 300, "bestBefore": "2026-09-12", "barcode": "22222222", "productName": "Closed tofu", "storage": "fridge"},
        ]}]
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_TOFU", "quantity": 100, "unit": "g", "consume": True}]
        )
        self.assertEqual(report["deductedLots"][0]["lotId"], "newer-printed")
        self.assertEqual(report["deductedLots"][0]["effectiveBestBefore"], "2026-09-09")
        self.assertEqual(report["deductedLots"][0]["barcode"], "11111111")
        self.assertEqual(after[0]["lots"][0]["id"], "newer-printed")
        self.assertEqual(after[0]["lots"][0]["quantity"], 200)

    def test_recipe_expiry_priority_uses_next_valid_batch_and_skips_past_batch(self):
        stock = [{"key": "M_FOOD_DATES", "name": "Dattel", "unit": "g", "lots": [
            {"quantity": 50, "bestBefore": "2026-09-06"},
            {"quantity": 150, "bestBefore": "2026-09-08"},
        ]}]
        recipe = {"ingredients": [{"foodKey": "M_FOOD_DATES", "foodName": "Datteln"}]}
        priority = self.module.recipe_expiry_priority(recipe, stock, today=date(2026, 9, 7), within_days=3)
    ...
    def test_recipe_consumption_matches_food_key_not_recipe_wording(self):
        stock = [{"key": "M_FOOD_SALMON", "name": "Lachsfilet", "quantity": 1, "unit": "kg", "bestBefore": "2026-09-09"}]
        recipe = {"title": "Test", "ingredients": [{
            "foodKey": "M_FOOD_SALMON", "foodName": "Lachsfilet", "name": "Lachsfilet (à 100 g)", "quantity": 200, "unit": "g"
        }]}
    ...
    def test_confirmation_deducts_recipe_amount_from_stock_unit(self):
        stock = [{"key": "M_FOOD_RICE", "name": "Rice", "quantity": 1, "unit": "kg"}]
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_RICE", "quantity": 200, "unit": "g", "consume": True}]
        )
    ...
    def test_consumption_uses_earliest_expiry_first_then_advances_date(self):
        stock = [{"key": "M_FOOD_BUTTER", "name": "Butter", "unit": "g", "lots": [
            {"quantity": 250, "bestBefore": "2026-09-10"},
            {"quantity": 500, "bestBefore": "2026-10-01"},
        ]}]
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_BUTTER", "quantity": 250, "unit": "g", "consume": True}]
        )
        self.assertEqual(after[0]["quantity"], 500)
        self.assertEqual(after[0]["bestBefore"], "2026-10-01")
        self.assertEqual([lot_signature(x) for x in after[0]["lots"]], [(500, "2026-10-01")])
    ...
    def test_partial_old_batch_keeps_old_expiry_until_that_batch_is_empty(self):
        ...
        after, _report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_BUTTER", "quantity": 100, "unit": "g", "consume": True}]
        )
    ...
    def test_cross_batch_consumption_continues_into_next_expiry(self):
        ...
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_BUTTER", "quantity": 300, "unit": "g", "consume": True}]
        )
    ...
    def test_depleted_stock_is_removed(self):
        ...
        after, report = self.module.apply_consumption(
            stock, [{"identity": "k:M_FOOD_EGG", "quantity": 2, "unit": "", "consume": True}]
        )
