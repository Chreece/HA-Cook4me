...
SCRIPT = Path(__file__).resolve().parents[1] / "tools/deploy_offline_runtime_v90.sh"
COMMIT = "55571e2c0626d5d08f0f0e5883760118b4410cb0"
...
if name == "git":
    if args[0] == "init":
        ...
        # This historical installer validates the v90 evidence count.
        path = staged / "catalog/price_portions.v1.json"
        payload = json.loads(path.read_text()); payload["portions"] = payload["portions"][:87]
        path.write_text(json.dumps(payload))
        (staged / "frontend").mkdir(parents=True)
        (staged / "frontend/cook4me-panel-v90-bundle.js").write_text("new panel\n")
        ...
        print("55571e2c0626d5d08f0f0e5883760118b4410cb0")
elif name == "docker":
    if args[0] == "inspect":
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_success_keeps_backup_and_installs_pinned_complete_component(self):
        ...
        runs = list((root / "config/.cook4me-deploy").glob("v90-*"))
        ...
        self.assertEqual(result.stdout.count("Offline price snapshot: 471 observations"), 2)
    ...
    def test_missing_offline_price_file_leaves_installation_untouched(self):
        for scenario in ("missing_prices", "missing_retail", "missing_density", "missing_reference_portions"):
