...
SCRIPT = Path(__file__).resolve().parents[1] / "tools/deploy_offline_runtime_v62.sh"
COMMIT = "66d813be424447c42bbdd00168264e5f1264d4de"
...
if name == "git":
    if args[0] == "init":
        ...
        (staged / "frontend/cook4me-panel-v62-bundle.js").write_text("new panel\n")
        (staged / "query_vocabulary.json").write_text("{}")
        (staged / "installed.txt").write_text("new")
    elif "rev-parse" in args:
        print("66d813be424447c42bbdd00168264e5f1264d4de")
elif name == "docker":
    if args[0] == "inspect":
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_success_keeps_backup_and_installs_pinned_complete_component(self):
        ...
        runs = list((root / "config/.cook4me-deploy").glob("v62-*"))
    ...
    def test_translation_failure_leaves_installation_and_container_untouched(self):
        result, _, target, calls = self.run_install("translation_failure")
