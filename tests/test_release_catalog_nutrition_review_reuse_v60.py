...
REUSE_FILE = TOOLS / "release_catalog_reviewed_nutrition_targets_008.v1.json"
EXPECTED_DIGEST = "ebf6eadb0fdf24560a20b9149a5bf2dbe9eb6c2df23ebbb43236bbaa53c23d99"
...
class NutritionReviewedDecisionReuseV60Tests(unittest.TestCase):
    def test_reuse_artifact_is_locked(self):
        ...
        self.assertEqual(len({row["reviewTargetId"] for row in items}), 26)
        self.assertEqual(_digest(items), EXPECTED_DIGEST)
        self.assertEqual(sum(int(row["usageCountAtReview"]) for row in items), 1925)
        self.assertEqual(sum(row["fdcDataType"] == "SR Legacy" for row in items), 14)
        self.assertEqual(sum(row["fdcDataType"] == "Foundation" for row in items), 9)
        self.assertEqual(sum(row["fdcDataType"] == "Survey (FNDDS)" for row in items), 3)
    ...
    def test_every_reuse_has_one_unambiguous_prior_exact_canonical_decision(self):
        ...
        for row in current:
            with self.subTest(target_id=row["reviewTargetId"], canonical=row["canonicalEnglishName"]):
                ...
                self.assertEqual(prior_fdc_ids, {int(row["fdcId"])}, "canonical review history must be unambiguous")
