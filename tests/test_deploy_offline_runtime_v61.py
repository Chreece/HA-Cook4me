...
if name == "git":
    if args[0] == "init":
        ...
        (staged / "frontend/cook4me-panel-v61.js").write_text("new panel\n")
...
class DeploymentTests(unittest.TestCase):
    def run_install(self, scenario):
        ...
        bin_dir = root / "bin"
        bin_dir.mkdir()
        for name in ("git", "docker", "curl", "mv"):
    ...
    def test_success_keeps_backup_and_installs_pinned_complete_component(self):
        ...
        runs = list((root / "config/.cook4me-deploy").glob("v61-*"))
    ...
    def test_preflight_failure_leaves_installation_and_container_untouched(self):
        result, _, target, calls = self.run_install("preflight_failure")
