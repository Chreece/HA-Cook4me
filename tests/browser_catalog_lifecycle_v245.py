"""Batch 29: avocado filters, supermarket names and exact-package opening controls."""
...
from test_catalog_lifecycle_v245 import PACKAGES, MONTHS, FROZEN, BOILED, NAMES, actual_choice, catalog, opening
...
def run():
    ...
    for _, ident, brand, code, rule, _, high, _ in PACKAGES:
        excluded = FROZEN if ident == 'M_FOOD_589' else BOILED
        for target, barcode, maker in ((ident, code, brand), (excluded, code, brand),
                                       (ident, code, 'Other'), (ident, '9002600274318', brand)):
            lot = {'brand': maker, 'barcode': barcode}
            scenarios.append({'ingredient': actual_choice(catalogs['el'], target), 'lot': lot, 'rule': rule,
                              'days': high, 'rules': opening.opening_rules({'ingredientId': target}, lot)})
        ...
        for width, height in ((390, 844), (1440, 980)):
            ...
            for country in ('DE', 'GR'):
                ...
                for month in range(1, 13):
                    ...
                    expect(option('M_FOOD_24')).to_have_count(int(country != 'GR' or month in MONTHS))
                    for ident in ids - {'M_FOOD_24'}:
            ...
            print(f'PASS {width}: twelve-month regional avocado filters, supermarket names, exact edamame/bamboo rules, form exclusions and opening confirmation')
            page.close()
        browser.close()
