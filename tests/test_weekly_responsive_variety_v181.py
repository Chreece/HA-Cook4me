...
class WeeklyResponsiveVarietyV181Tests(unittest.TestCase):
    ...
    def test_weekly_candidate_work_is_bounded_and_scored_off_event_loop(self):
        ...
        self.assertIn("_week_candidate_nutrition",generate)
        self.assertIn("nutrition_by_id.update(",generate)
        ...
        week_candidates=_function_source(WEEK,"_week_candidates")
        self.assertIn("await hass.async_add_executor_job(",week_candidates)
        self.assertIn("v13._rank_filtered",week_candidates)
        score=_function_source(WEEK,"_score_week_pool")
        self.assertEqual(score.count("calculate_recipe_cost("),1)
        self.assertIn("nutrition_by_id.get(id(candidate))",score)
    ...
    def test_week_job_has_real_progress_including_final_per_slot_refresh(self):
        ...
        self.assertIn("Scoring candidate {candidate_index}/{pool_total}",week)
        self.assertIn("progress=score_progress",week)
        self.assertIn("progress=progress",week)
        self.assertIn("reuse_costs=True",week)
