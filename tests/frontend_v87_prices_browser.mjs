...
try{
 ...
 await page.goto('http://cook4me.test/');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v87-bundle.js'});
 await page.evaluate(()=>{
  globalThis.customElements.define('cook4me-v87-test',class extends customElements.get('cook4me-recipe-hub-panel-v87'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v87-test');
 ...
 assert.match(await page.locator('cook4me-v87-test [data-v82-card-cost]').first().innerText(),/0\.22/);
 assert.match(await page.locator('cook4me-v87-test [data-v82-card-cost]').first().innerText(),/1\/1 ingredients priced/);
 await page.evaluate(()=>{
 ...
 assert.match(await page.locator('cook4me-v87-test [data-v82-card-cost]').first().innerText(),/Subtotal.*2.12.*1\/2 ingredients priced/);
 assert.match(await page.locator('cook4me-v87-test [data-v82-card-cost]').first().getAttribute('title'),/Salt <unsafe>.*quantity missing/);
 assert.equal(await page.locator('cook4me-v87-test unsafe').count(),0);
 ...
 const panel=page.locator('cook4me-v87-test');
 ...
 for(const url of ['javascript:alert(1)','https://www.dm.de.evil.test/x','https://www.dm.de@evil.test/x']){
 ...
 assert.deepEqual(errors,[]);console.log('v87 browser: offline previews, concurrency, entry isolation, quantity sources and missing-cost explanation passed');
