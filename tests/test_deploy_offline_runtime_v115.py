...
SCRIPT = Path(__file__).resolve().parents[1] / "tools/deploy_offline_runtime_v115.sh"
COMMIT = "7d99d82d935bc00b21fe47744292fbec829657d4"
...
if name == "git":
    if args[0] == "init":
        ...
        (staged / "frontend/cook4me-panel-v115-bundle.js").write_text("new panel\n")
        if scenario == "missing_prices":
            (staged / "catalog/observed_prices.v1.json").unlink()
        if scenario == "missing_units":
            (staged / "catalog/ui_units.v1.json").unlink()
        if scenario == "missing_allowances":
            (staged / "price_allowances.py").unlink()
        ...
        if scenario == "missing_reference_portions":
            (staged / "catalog/price_reference_portions.v1.json").unlink()
        if scenario == "missing_benchmarks":
            (staged / "catalog/price_benchmarks.v1.json").unlink()
        if scenario == "missing_density":
            (staged / "catalog/price_densities.v1.json").unlink()
        ...
        print("7d99d82d935bc00b21fe47744292fbec829657d4")
elif name == "docker":
    if args[0] == "inspect":
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_success_keeps_backup_and_installs_pinned_complete_component(self):
        ...
        runs = list((root / "config/.cook4me-deploy").glob("v115-*"))
        ...
        self.assertEqual(result.stdout.count("Offline price snapshot: 520 observations"), 2)
        ...
        suites = [call[call.index("-p") + 1] for call in calls if "unittest" in call]
        self.assertEqual(suites, ["test_price_fixes_v104.py", "test_entity_setup_v73.py", "test_device_announcements_v72.py", "test_announcement_fallback_v105.py", "test_cross_language_send_v106.py", "test_dietary_guidance_v107.py", "test_delivery_confirmation_v108.py", "test_weekly_summary_v109.py", "test_weekly_selection_v110.py", "test_product_capture_v78.py", "test_scanner_v111.py", "test_product_packages_v112.py", "test_product_links_v114.py", "test_linked_stock_audit_v115.py"])
    ...
    def test_missing_offline_price_file_leaves_installation_untouched(self):
        for scenario in ("missing_prices", "missing_retail", "missing_density", "missing_reference_portions", "missing_benchmarks", "missing_allowances", "missing_units"):
