...
try{
 ...
 await page.goto('http://cook4me.test/');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v92-bundle.js'});
 await page.evaluate(()=>{
  globalThis.customElements.define('cook4me-v92-test',class extends customElements.get('cook4me-recipe-hub-panel-v92'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v92-test');
 ...
 assert.match(await page.locator('cook4me-v92-test [data-v82-card-cost]').first().innerText(),/0\.22/);
 assert.match(await page.locator('cook4me-v92-test [data-v82-card-cost]').first().innerText(),/1\/1 ingredients priced/);
 await page.evaluate(()=>{
 ...
 assert.match(await page.locator('cook4me-v92-test [data-v82-card-cost]').first().innerText(),/Subtotal.*2.12.*1\/2 ingredients priced/);
 assert.match(await page.locator('cook4me-v92-test [data-v82-card-cost]').first().getAttribute('title'),/Salt <unsafe>.*quantity missing/);
 assert.equal(await page.locator('cook4me-v92-test unsafe').count(),0);
 ...
 const panel=page.locator('cook4me-v92-test');
 ...
 assert.match(await panel.locator('[data-v82-card-cost]').innerText(),/Partial estimate.*2.50.*2\/3/);
 assert.match(await panel.locator('[data-v82-card-cost]').getAttribute('aria-label'),/1 rough estimates/);
 await page.evaluate(()=>{panel._uiIngredientLanguage=()=> 'el';panel._v82PaintCard(budgetRecipe);});
 assert.match(await panel.locator('[data-v82-card-cost]').innerText(),/Μερική εκτίμηση/);
 ...
 const installHarness=()=>{
  globalThis.customElements.define('cook4me-cache-test',class extends customElements.get('cook4me-recipe-hub-panel-v92'){connectedCallback(){}});
 ...
 await page.reload();await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v92-bundle.js'});await page.evaluate(installHarness);
 ...
 for(const changes of [{testToken:'token-two'},{testUser:'bob'}]){
  await page.reload();await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v92-bundle.js'});await page.evaluate(changes=>Object.assign(window,changes),changes);await page.evaluate(installHarness);
 ...
 assert.deepEqual(errors,[]);console.log('v92 browser: offline previews, concurrency, entry isolation, persistent reload cache, changed evidence, retries, quantity sources and missing-cost explanation passed');
