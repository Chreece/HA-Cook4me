...
const fixture=JSON.parse(readFileSync(root+'/docs/price-expansion-v101-preview.json','utf8'));
...
try{
 ...
 await page.goto('about:blank');await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v101-bundle.js'});
 await page.evaluate(fixture=>{
  globalThis.customElements.define('cook4me-v101-test',class extends globalThis.customElements.get('cook4me-recipe-hub-panel-v101'){connectedCallback(){} disconnectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v101-test');p._hass={language:'el'};p._uiIngredientLanguage=()=>p._hass.language;p._langCode=()=>p._hass.language;
 ...
 const p=page.locator('cook4me-v101-test');
 for(const language of ['en','de','el']){
  ...
  assert.match(await p.evaluate(node=>node.shadowRoot.textContent),/23[,.]28/);
  assert.equal(await page.evaluate(()=>state.cost.priceConfidence),'reference');
 }
 for(const host of ['www.der-ludwig.de','www.send-a-fish.de','www.naturata-shop.de','shop.vanillekiste.de']){
 ...
 assert.deepEqual(errors,[]);console.log('v101: actual offline costs and five package references render in English, German and Greek; reviewed source links validate correctly');
