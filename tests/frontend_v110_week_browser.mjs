...
try{
 ...
 await page.addScriptTag({path:root+'/custom_components/cook4me/frontend/cook4me-panel-v110-bundle.js'});
 await page.evaluate(fixture=>{
  window.fixture=fixture;window.requests=[];window.subscriptions=[];
  globalThis.customElements.define('cook4me-v110-test',class extends globalThis.customElements.get('cook4me-recipe-hub-panel-v110'){connectedCallback(){}});
  const p=window.panel=document.createElement('cook4me-v110-test');
  ...
  window.respond=async request=>{
   ...
   if(request.type.endsWith('/week_select')){
    if(window.failSelection)throw Error('selection save failed');
    for(const slot of plan.slots)if(request.slot_ids.includes(slot.id))slot.selected=request.selected;
   }
   if(request.type.endsWith('/week_state')||request.type.endsWith('/week_generate')||request.type.endsWith('/week_select')){
    ...
    copy.filteredSlotCount=plan.slots.length-copy.slots.length;
    const selected=copy.slots.filter(row=>row.selected!==false&&!row.leftoverId),required=selected.length*100;
    copy.reservations={items:required?[
     {name:'Rice',quantity:required,unit:'g',available:150,shortage:Math.max(0,required-150)},
     {name:'Water',quantity:required,unit:'ml',available:required,shortage:0,unlimited:true},
     {name:'Unknown salt',quantity:1,unit:'g',available:null,shortage:null}
    ]:[],unknown:required?[{name:'Unknown salt',quantity:1,unit:'g'}]:[]};
    copy.shoppingDelta=required>150?[{name:'Rice',quantity:required-150,unit:'g'}]:[];
    return copy;
   }
   return {};
  };
 ...
 const p=page.locator('cook4me-v110-test');
 ...
 // Real checkbox clicks persist selection; day checkbox has a mixed state.
 const settled=()=>page.waitForFunction(()=>!panel._v110Busy&&!panel._weekLoading);
 assert.equal(await p.locator('[data-week-select]:checked').count(),4);
 assert.equal(await p.locator('[data-week-day-select]').count(),7);
 assert.equal(await p.locator('#generateWeek ha-icon').getAttribute('icon'),'mdi:calendar-plus');
 assert.equal(await p.locator('[data-week-action]').count(),4);
 await p.locator('[data-week-disclosure=stock] summary').click();
 await p.locator('[data-week-disclosure=shopping] summary').click();
 assert.equal(await p.locator('[data-week-disclosure=stock] .rx-list-row').count(),1,'Only Water is fully covered');
 assert.match(await p.locator('[data-week-disclosure=shopping]').innerText(),/150/);
 await p.locator('[data-week-select=b]').uncheck();await settled();
 assert.equal(await p.locator('[data-week-day-select="2026-09-17"]').evaluate(n=>n.indeterminate),true);
 assert.equal((await totals()).count,3);
 assert.equal((await totals()).values[0].value,320);
 assert.deepEqual((await totals()).cost,{EUR:10});
 assert.equal(await p.locator('[data-week-disclosure=stock]').getAttribute('open'),'');
 assert.match(await p.locator('[data-week-disclosure=shopping]').innerText(),/50/);
 await p.locator('[data-week-select=d]').uncheck();await settled();
 assert.equal(await p.locator('[data-week-disclosure=stock] .rx-list-row').count(),2,'Rice becomes fully covered');
 assert.equal(await p.locator('#addPlanShopping').isDisabled(),true);
 await p.locator('[data-week-action=regenUnselected]').click();await settled();
 assert.deepEqual(await page.evaluate(()=>requests.findLast(r=>r.type.endsWith('/week_generate')).replace_slot_ids),['b','d']);
 await p.locator('[data-week-action=regenSelected]').click();await settled();
 assert.deepEqual(await page.evaluate(()=>requests.findLast(r=>r.type.endsWith('/week_generate')).replace_slot_ids),['a','c']);
 assert.equal(await p.locator('[data-week-select]:checked').count(),2,'Regeneration preserves selection');
 await p.locator('[data-week-day-select="2026-09-17"]').check();await settled();
 assert.equal(await p.locator('[data-week-select=b]').isChecked(),true);
 await p.locator('[data-week-day-select="2026-09-17"]').uncheck();await settled();
 assert.equal(await p.locator('[data-week-select=a]').isChecked(),false);
 assert.equal(await p.locator('[data-week-select=b]').isChecked(),false);
 await p.locator('[data-week-action=none]').click();await settled();
 assert.equal((await totals()).count,0);assert.equal((await totals()).values[0].value,0);
 assert.deepEqual((await totals()).cost,{EUR:0});
 assert.equal(await p.locator('[data-week-disclosure=stock] .rx-list-row').count(),0);
 assert.equal(await p.locator('[data-week-action=regenSelected]').isDisabled(),true);
 assert.equal(await p.locator('[data-week-action=regenUnselected]').isDisabled(),false);
 // Failed mutation rolls back the checkbox; reloading preserves saved choices.
 await page.evaluate(()=>failSelection=true);
 await p.locator('[data-week-select=a]').click();await settled();
 assert.equal(await p.locator('[data-week-select=a]').isChecked(),false);
 await page.evaluate(()=>{failSelection=false;return panel._loadWeekState(true);});
 assert.equal(await p.locator('[data-week-select]:checked').count(),0);
 await p.locator('[data-week-action=all]').click();await settled();
 assert.equal(await p.locator('[data-week-select]:checked').count(),4);
 assert.equal(await p.locator('[data-week-action=all]').isDisabled(),true);
 assert.equal((await totals()).values[0].value,1220);
 ...
 await settled();
 await p.locator('[data-week-action=none]').click();await settled();
 assert.equal(await page.evaluate(()=>plan.slots.find(row=>row.id==='a').selected),true,'Select all/none acts only on filtered visible recipes');
 await p.locator('[data-week-action=all]').click();await settled();
 ...
 for(const language of ['el','de','en']){
  ...
  for(const width of [390,1600]){
   ...
   if(process.env.COOK4ME_SCREENSHOT_DIR&&language==='el'){
    await page.screenshot({path:process.env.COOK4ME_SCREENSHOT_DIR+`/v110-week-${width}.png`,fullPage:true});
    await card.screenshot({path:process.env.COOK4ME_SCREENSHOT_DIR+`/v110-week-summary-${width}.png`});
   }
  }
 }
 assert.deepEqual(errors,[]);
 console.log('v110: persistent per-recipe/day/all selection, mixed state, bulk targets, zero totals, covered stock, shortages, failed save rollback; planned quantities, seven future dates, partial/missing/zero values, live budget totals, shared filter Apply/generate/clear, stale responses, retry recovery and el/de/en responsive layout passed');
