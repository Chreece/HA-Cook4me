...
class Cook4MeRecipeHubPanelV83 extends BasePanel{
 ...
 _v83ExclusionPicker(c,row,onchange,openSet=new Set(),key='filter'){
  ...
  const details=c.querySelector('details');details.addEventListener('toggle',()=>{if(details.open){openSet.add(key);fill();if(!this._ingredientCatalog?.length&&!this._ingredientCatalogLoading){void this._loadIngredientCatalog(null,true).then(()=>{if(c.isConnected)fill();});}}else openSet.delete(key);});
