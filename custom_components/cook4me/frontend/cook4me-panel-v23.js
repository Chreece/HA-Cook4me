...
const TEXT = {
  en:{
    ...
    nutritionSettings:"USDA fallback nutrition",fdcApiKey:"USDA FoodData Central API key",fdcHelp:"Optional fallback only. Cook4Me keeps reviewed/exact nutrition separate; USDA is used to fill otherwise unknown generic ingredients. Without your own key it uses USDA DEMO_KEY with low rate limits.",
    saveApiKey:"Save API key",clearApiKey:"Clear API key",apiKeyConfigured:"Personal USDA API key configured",demoKeyMode:"Using USDA DEMO_KEY",buildCatalog:"Try next USDA fallback batch",catalogMapped:"fallback ingredients mapped",catalogRemaining:"fallback candidates remaining",scannedNutrition:"Nutrition captured from scanned product",
  },
  de:{
    ...
    nutritionSettings:"USDA-Nährwert-Fallback",fdcApiKey:"USDA FoodData Central API-Schlüssel",fdcHelp:"Nur optionaler Fallback. Geprüfte/exakte Nährwerte bleiben getrennt; USDA ergänzt nur sonst unbekannte generische Zutaten. Ohne eigenen Schlüssel wird USDA DEMO_KEY mit niedrigen Limits verwendet.",
    saveApiKey:"API-Schlüssel speichern",clearApiKey:"API-Schlüssel löschen",apiKeyConfigured:"Persönlicher USDA-API-Schlüssel eingerichtet",demoKeyMode:"USDA DEMO_KEY wird verwendet",buildCatalog:"Nächsten USDA-Fallback-Block versuchen",catalogMapped:"Fallback-Zutaten zugeordnet",catalogRemaining:"Fallback-Kandidaten verbleiben",scannedNutrition:"Nährwerte vom gescannten Produkt übernommen",
  },
  el:{
    ...
    nutritionSettings:"Εφεδρικά διατροφικά στοιχεία USDA",fdcApiKey:"Κλειδί API USDA FoodData Central",fdcHelp:"Προαιρετικό fallback μόνο. Τα ελεγμένα/ακριβή διατροφικά στοιχεία παραμένουν ξεχωριστά· το USDA συμπληρώνει μόνο γενικά υλικά που αλλιώς μένουν άγνωστα. Χωρίς προσωπικό κλειδί χρησιμοποιείται το USDA DEMO_KEY με χαμηλά όρια.",
    saveApiKey:"Αποθήκευση κλειδιού API",clearApiKey:"Διαγραφή κλειδιού API",apiKeyConfigured:"Έχει ρυθμιστεί προσωπικό κλειδί USDA API",demoKeyMode:"Χρήση USDA DEMO_KEY",buildCatalog:"Δοκιμή επόμενης παρτίδας USDA fallback",catalogMapped:"υλικά fallback αντιστοιχισμένα",catalogRemaining:"υποψήφια fallback απομένουν",scannedNutrition:"Τα διατροφικά στοιχεία λήφθηκαν από το σαρωμένο προϊόν",
  },
};
