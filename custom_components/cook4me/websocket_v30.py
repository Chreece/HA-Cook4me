...
def _materialize_reference_recipe(raw: dict[str, Any], *, ui_language: str, source_language: str, device_language: str) -> dict[str, Any]:
    ...
    display_variant = catalog.variant(
        recipe_id,
        language=source_language or ui_language,
        fallback_language=ui_language,
    )
    send_variant = catalog.variant(
        recipe_id,
        language=device_language,
        fallback_language=source_language or ui_language,
    )
    ...
    for raw_ingredient in full.get("ingredients") or []:
        if not isinstance(raw_ingredient, dict):
            continue
        ingredient_id = _text(
            raw_ingredient.get("id")
            or raw_ingredient.get("key")
            or raw_ingredient.get("foodKey")
        )
        ...
        item: dict[str, Any] = {
            ...
            "key": ingredient_id,
            "foodName": _text(
                (ingredient or {}).get("name")
                or (ingredient or {}).get("canonicalName")
                or ingredient_id
            ),
        }
    ...
    result: dict[str, Any] = {
        ...
        "groupSize": full.get("servings") or full.get("groupSize"),
        "courses": deepcopy(full.get("courses") or []),
        "occasions": deepcopy(full.get("occasions") or []),
        "durations": deepcopy(full.get("durations") or {}),
        "difficulty": full.get("difficulty"),
        "recipeType": full.get("recipeType"),
        "source": "bundled_reference_catalog",
        "referenceCatalogVersion": catalog.metadata.get("catalogVersion"),
        "language": _text(
            (display_variant or {}).get("language")
            or source_language
            or ui_language
        ),
    ...
    if display_variant:
        ...
        if variant_id:
            ...
            result["variantFunctionalId"] = _text(
                display_variant.get("recipeFunctionalId") or variant_id
            )
    ...
    if send_variant:
        ...
        if send_id:
            ...
            result["sendRecipeFunctionalId"] = _text(
                send_variant.get("recipeFunctionalId") or send_id
            )
...
def reference_recipe_candidates(
    bridge,
    *,
    languages: list[str],
    query: str,
    ...
    for row in rows:
        ...
        available_languages = {
            _language(variant.get("language"))
            for variant in variants
            if _text(variant.get("language"))
        }
        for source_language in languages:
            if source_language not in available_languages:
                continue
            item = _materialize_reference_recipe(
                row,
                ui_language=ui_language,
                source_language=source_language,
                device_language=device_language,
            )
            item["sourceCatalogLanguage"] = source_language
            item["todayCatalogLanguage"] = source_language
            item["officialCatalogLanguage"] = source_language
            out.append(item)
    return out
...
def async_register(hass: HomeAssistant) -> None:
    for command in (
        ws_ui_seed,
        ws_reference_meta,
        ws_reference_ingredients,
        ws_reference_recipes,
        ws_progress_state,
    ):
...
@websocket_api.websocket_command(
    {
        ...
        vol.Optional("limit", default=5000): vol.All(
...
def ws_reference_ingredients(hass, connection, msg) -> None:
    try:
        ...
        items = catalog.ingredients(
            language=_language(msg.get("language")),
...
def ws_reference_recipes(hass, connection, msg) -> None:
    try:
        ...
        selected = [
            _language(value)
            for value in msg.get("languages") or []
            if _text(value)
        ]
        if not selected:
            selected = [v11._device_language(bridge)]
        items = reference_recipe_candidates(
            bridge,
            languages=selected,
            query=_text(msg.get("query")),
            ui_language=_language(msg.get("language")),
...
@websocket_api.websocket_command(
    {vol.Required("type"): "cook4me/v30/progress_state"}
)
@websocket_api.async_response
